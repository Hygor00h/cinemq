...
public class PagamentoController {
	...
	public ResponseEntity<String> simularPagamento(@RequestBody @Valid PagamentoDTO dto) {
		...
		if (StatusProcesso.SUCESSO.equals(compra.getStatus())) {
			return ResponseEntity.badRequest().body("Este pedido já foi pago!");
		}
		if (!StatusProcesso.AGUARDANDO_PAGAMENTO.equals(compra.getStatus())) {
		...
		if (dto.getValor() == null || dto.getValor().compareTo(valorEsperado) != 0) {
			compra.setStatus(StatusProcesso.PAGAMENTO_INCORRETO);
		...
		compra.setStatus(StatusProcesso.SUCESSO);
